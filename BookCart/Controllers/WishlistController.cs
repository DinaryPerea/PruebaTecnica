...
namespace BookCart.Controllers
{
    ...
    public class WishlistController : Controller
    {
        ...
        readonly IProductoService _bookService;
        readonly IUserService _userService;

        public WishlistController(IWishlistService wishlistService, IProductoService bookService, IUserService userService)
        {
        ...
        [HttpGet("{userId}")]
        public async Task<List<Producto>> Get(int userId)
        {
        ...
        public async Task<List<Producto>> Post(int userId, int bookId)
        {
        ...
        List<Producto> GetUserWishlist(int userId)
        {
            bool user = _userService.isUserExists(userId);
            if (user)
            {
                string Wishlistid = _wishlistService.GetWishlistId(userId);
                return _bookService.GetProductosAvailableInWishlist(Wishlistid);
            }
            else
            {
